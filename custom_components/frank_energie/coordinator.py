...
from .api import FrankEnergieApi, AuthException, RequestException
from .const import CONF_SITE_REFERENCE, DATA_ELECTRICITY, DATA_GAS, DATA_MONTH_SUMMARY, DATA_INVOICES
from .models import PriceData, MonthSummary, Invoices, MarketPrices
...
class FrankEnergieCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        self.entry = entry
        self.api = api
        self.site_reference = entry.data.get(CONF_SITE_REFERENCE)
    ...
    async def _async_update_data(self) -> FrankEnergieData:
        ...
        try:
            ...
            # Check if we still have data to work with
            if (
                self.data
                and self.data.get(DATA_ELECTRICITY)
                and self.data[DATA_ELECTRICITY].upcoming
                and self.data.get(DATA_GAS)
    ...
    async def __try_renew_token(self):
        try:
            ...
            data = {
                **self.entry.data,
                CONF_ACCESS_TOKEN: updated_tokens.get("authToken"),
                CONF_TOKEN: updated_tokens.get("refreshToken"),
            }
